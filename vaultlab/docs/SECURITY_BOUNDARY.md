...
8. Policy decisions never perform cryptographic operations, activate or distribute clients or disclosure programs, onboard providers, execute contracts/payments, issue credentials, share data, execute updates, access devices or raw data, store or generate/export keys, publish prices or disclosures, trade, execute orders, mutate risk limits or retention, scan or exploit systems, generate traffic, run chaos, delete data, execute API requests, grant access, start sessions, run ceremonies, activate signers, read/write/delete audit records, revoke credentials, delete artifacts, grant security exceptions, bypass policy, remediate, patch, restore, fail over, mutate data or balances, make financial claims, hold, contain, execute revocation, withdraw, sign, deploy, activate custody, or move assets.
