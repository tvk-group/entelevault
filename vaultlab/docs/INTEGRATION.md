...
- assurance-signer gateway evidence contains only allowlisted workload identity metadata, receipt metadata, digests, revision, classifications, booleans, and byte limits; it accepts no runtime bearer token, arbitrary payload, key material, or signature and cannot execute or sign a request;
