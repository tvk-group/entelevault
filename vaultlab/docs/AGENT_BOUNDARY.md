...
An independent control service—not the agent—may implement a reversible quarantine or JIT grant after its own authorization policy. Client activation/distribution, update execution, device or raw-data access, key storage, price publication, trading, order execution, risk-limit or retention mutation, vulnerability scanning, exploitation, traffic generation, chaos execution, data deletion, patching, API execution, access, session creation, revocation execution, credential rotation, artifact deletion, audit read/write/delete, policy exceptions, remediation, signer ceremonies, key generation, signer activation, restoration, failover, data/balance mutation, signing, deployment, recovery, withdrawal, and asset movement require separate quorum-controlled systems.
