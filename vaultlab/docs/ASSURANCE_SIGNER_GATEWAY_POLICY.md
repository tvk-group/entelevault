...
Review eligibility requires the fixed `osoix.assurance-signing-request.v1` contract, `POST`, the exact assurance-signing purpose, Ed25519 plus ML-DSA-65 algorithm intent, an exact source revision, a recomputed receipt digest, a fresh five-minute deny receipt, no command path, TLS 1.3, redirect rejection, JSON-only bounded bodies, Trusted Sources evidence, unique replay and idempotency state, enforced rate limits, hardware-backed non-exportable purpose-isolated keys, an available audit sink, and independent provider assessment.

The current policy deliberately permits only `deny` receipts to become review-eligible. An `allow` receipt, generic signing purpose, arbitrary payload, key-purpose confusion, replay, source mismatch, provider downgrade, or missing evidence is blocked.
